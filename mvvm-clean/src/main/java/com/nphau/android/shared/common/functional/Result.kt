...
sealed class Result<out L, out R> {
    ...
    fun oneOf(onSuccess: (L) -> Any, onError: (R) -> Any): Any =
        when (this) {
...
fun <L, R> Result<L, R>.get(): L = (this as Result.Success).data

fun <L, R> Result<L, R>.exception(): R {
    return (this as? Result.Failure)?.error!!
}

fun <L, R> Result<L, R>.exceptionOrNull(): R? {
    return (this as? Result.Failure)?.error
}